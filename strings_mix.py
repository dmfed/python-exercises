...
only take into account the lowercase letters (a to z). First let us count the frequency of 
...
def mix(s1, s2):
    clean1 = set(filter(lambda a: a.islower() and s1.count(a) > 1, s1))
    clean2 = set(filter(lambda a: a.islower() and s2.count(a) > 1, s2))
    out =[]
    for letter in (clean1 | clean2):
        count1, count2 = s1.count(letter), s2.count(letter)
        if count1 == count2:
            out.append(f"=:{letter * count1}")
        elif count1 > count2:
            out.append(f"1:{letter * count1}")
        else:
            out.append(f"2:{letter * count2}")
...
s1 = "my&friend&Paul has heavy hats! &"
s2 = "my friend John has many many friends &"
assert mix(s1, s2) == "2:nnnnn/1:aaaa/1:hhh/2:mmm/2:yyy/2:dd/2:ff/2:ii/2:rr/=:ee/=:ss"

s1="Are the kids at home? aaaaa fffff"
s2="Yes they are here! aaaaa fffff"
assert mix(s1, s2) == "=:aaaaaa/2:eeeee/=:fffff/1:tt/2:rr/=:hh"
